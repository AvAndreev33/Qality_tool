"""Envelope area metric for Quality_tool.

Computes the total area (sum) of the envelope::
...
class EnvelopeArea:
    """Envelope area metric.

    Score meaning: higher is better.
    """

    name: str = "envelope_area"
    category: str = "envelope"
    display_name: str = "Envelope Area"
    ...
    def evaluate(
        ...
        ea = float(np.sum(envelope))
        return MetricResult(score=ea, features={"envelope_area": ea})
