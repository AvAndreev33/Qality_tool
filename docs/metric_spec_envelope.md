...
- `analysis_context.beta_secondary_peak`
...
4. Keep only significant secondary peaks:
   a secondary peak with height e_sec is significant if
   e_sec >= β * e_peak

5. Score:
   NSSP = number of retained significant secondary peaks
```
...
## Notes
- `α = analysis_context.alpha_main_support`
- `β = analysis_context.beta_secondary_peak`
- If secondary peak detection uses explicit thresholds, they should come from:
  - `analysis_context.secondary_peak_min_distance`
  - `analysis_context.secondary_peak_min_prominence`
- This metric counts clearly expressed secondary envelope peaks outside the main packet region.
- It is useful for identifying fragmented or multi-peak packet structure.